...
import { FaCoins, FaMoneyBill, FaStar } from 'react-icons/fa';
...
export const UserMenuItems = ({ user, setMobileMenuOpen }: { user?: Partial<User>; setMobileMenuOpen?: any }) => {
  ...
  const generateImagePath = routes.GenerateImageRoute.to;
    ...
    <>
      ...
      >
        {path === pathsToShowImageGenerator[path] ? (
          <li>
            <WaspRouterLink to={generateImagePath} className='flex items-center gap-3.5 text-sm font-medium duration-300 ease-in-out hover:text-yellow-500'>
        ...
        <li>
          <WaspRouterLink to={routes.PricingPageRoute.to} onClick={handleMobileMenuClick} className='flex items-center gap-3.5 text-sm font-medium duration-300 ease-in-out hover:text-yellow-500'>
            <FaCoins size='1.1rem' />
      ...
      {!!user && user.isAdmin && (
        ...
        >
          <li className='flex items-center gap-3.5 text-sm font-medium duration-300 ease-in-out hover:text-yellow-500'>
            <WaspRouterLink to={routes.AdminRoute.to} onClick={handleMobileMenuClick} className='flex items-center gap-3.5 text-sm font-medium duration-300 ease-in-out hover:text-yellow-500'>
      ...
      <button
        onClick={() => logout()}
        className={cn('flex items-center gap-3.5 py-4 text-sm font-medium duration-300 ease-in-out hover:text-yellow-500', {
          'sm:px-6': path !== adminDashboardPath,
          'px-6': path === adminDashboardPath,
        })}
      >
